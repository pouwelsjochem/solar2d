...
bool WinTimer::Init_V2()
{
	...
	fIntermediateStops = false;
	fStartOrStopEventOK = false;
	...
	for (int i = kFirstEvent; i < kNumHandles; i++)
	{
		fHandles[i] = CreateEvent(NULL, FALSE, FALSE, NULL);
...
// TODO: SetThreadPriority with THREAD_PRIORITY_HIGHEST
...
void WinTimer::Start_V2()
{
	...
	{
		if (fStartOrStopEventOK)
		{
			SetEvent(fHandles[kStartedEvent]);
		}
...
void WinTimer::Stop_V2(LONG stopState)
{
	...
	if (normalStop || (kQuitting == stopState && LockFromExpectedStateTo(kStopped, kQuitting)))
	{
		...
		bool stopEventOK = fStartOrStopEventOK;

		fIntermediateStops = true;
		fStartOrStopEventOK = false;
		...
		if (stopEventOK)
		{
			SetEvent(fHandles[kStoppedEvent]);
		}
	}
// TODO: restore period?
}
...
DWORD WinTimer::WaitForEventOrStop(EventID eventID)
{
	HANDLE events[] = { fHandles[eventID], fHandles[kStoppedEvent] };
...
bool WinTimer::WaitWhileStopped()
{
	...
	{
		fStartOrStopEventOK = true;

		InterlockedExchange(&fState, kStopped); // unlock and wait
		
		DWORD waitResult = WaitForEventOrStop(kStartedEvent);
		
		return WAIT_OBJECT_0 == waitResult; // a "stop" here (event 1) means to quit
	}
	else
	{
...
void WinTimer::WaitForInterval()
{
	U64 now = Rtt_GetAbsoluteTime();

	Rtt_ASSERT(now >= fEvaluateBeganTime);

	if (fPeriodMin)
	{
		double ns = (double)(now - fEvaluateBeganTime);
// TODO: instead of divide, multiply by fps interval?
		PreciseSleep(fFPSInterval - fmod(ns / 1'000'000'000, fFPSInterval));
	}
	else
	{
		DWORD leftover = fIntervalInMilliseconds - Rtt_AbsoluteToMilliseconds(now - fEvaluateBeganTime) % fIntervalInMilliseconds;
		Sleep(leftover);
	}
}
...
VOID CALLBACK WinTimer::OnTimerElapsed_V2(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	...
	if (!timer->fIntermediateStops && kRunning == timer->fState) // n.b. in WM_TIMER, so no contention
	{
		...
		SetEvent(timer->fHandles[kEvaluateEvent]);
...
void WinTimer::ThreadBody()
{
	while (true)
	{
		...
		{
			bool hasInterval = InvalidBeganTime() != fEvaluateBeganTime;
		...
		if (TryToLockFromExpectedState(kRunning))
		{
			...
			// Spoof a WM_TIMER event...
			InterlockedExchange(&fState, kRunning); // unlock and wait
			PostMessage(fWindowHandle, WM_TIMER, (WPARAM)this, (LPARAM)&OnTimerElapsed_V2);
			...
			if (WAIT_OBJECT_0 == WaitForEventOrStop(kEvaluateEvent))
			{
				fStartOrStopEventOK = false;
			
				WaitForSingleObject(fHandles[kEvaluateEvent], INFINITE);
			}
		}
	}
}
