...
WinInputDeviceManager::WinInputDeviceManager(Interop::RuntimeEnvironment& environment)
	:	PlatformInputDeviceManager(&environment.GetAllocator()),
		...
		fRenderSurfaceLostFocusEventHandler(this, &WinInputDeviceManager::OnRenderSurfaceLostFocus),
		...
		fCursorStyle(WinInputDeviceManager::CursorStyle::kDefaultArrow)
{
	...
	if (surfacePointer)
	{
		...
		surfacePointer->GetLostFocusEventHandlers().Add(&fRenderSurfaceLostFocusEventHandler);
	}
}
...
WinInputDeviceManager::~WinInputDeviceManager()
{
	...
	if (surfacePointer)
	{
		...
		surfacePointer->GetLostFocusEventHandlers().Remove(&fRenderSurfaceLostFocusEventHandler);
	}
...
void WinInputDeviceManager::OnRenderSurfaceLostFocus(Interop::UI::Control& sender, const Interop::EventArgs& arguments)
{
	Rtt_UNUSED(sender);
	Rtt_UNUSED(arguments);
	CancelActiveTouches();
}
...
void WinInputDeviceManager::CancelActiveTouches()
{
	auto surfacePointer = fEnvironment.GetRenderSurface();
	if (surfacePointer && (::GetCapture() == surfacePointer->GetWindowHandle()))
	{
		::ReleaseCapture();
	}

	for (int index = 0; index < kMaxTouchPoints; index++)
	{
		auto touchPointStatePointer = &fTouchPointStates[index];
		if (!touchPointStatePointer->HasStarted)
		{
			continue;
		}
		...
		touchPointStatePointer->HasStarted = false;
		touchPointStatePointer->TouchInputId = 0;
		OnReceivedTouchEvent(
				(uint32_t)index, touchPointStatePointer->LastPoint,
				touchPointStatePointer->StartPoint, Rtt::TouchEvent::kCancelled);
	}
