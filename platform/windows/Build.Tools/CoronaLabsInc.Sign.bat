...
if NOT EXIST "%~dp0TRAGsoft.pfx" goto :OnSkipSigning
...
signtool sign /fd sha1 /f "%~dp0TRAGsoft.pfx" /p "%WIN_CERT_PASSWORD%" /t http://timestamp.digicert.com %programNameArgument% "%~1"
if errorlevel 0 goto:eof
echo "Signin failed. Using fallback timestamp server 1!"
signtool sign /fd sha1 /f "%~dp0TRAGsoft.pfx" /p "%WIN_CERT_PASSWORD%" /t http://timestamp.verisign.com/scripts/timestamp.dll %programNameArgument% "%~1"
if errorlevel 0 goto:eof
echo "Signin failed. Using fallback timestamp server 2!"
signtool sign /fd sha1 /f "%~dp0TRAGsoft.pfx" /p "%WIN_CERT_PASSWORD%" /t http://timestamp.globalsign.com/scripts/timestamp.dll %programNameArgument% "%~1"
if errorlevel 0 goto:eof
echo "Signin failed. Using fallback timestamp server 3!"
signtool sign /fd sha1 /f "%~dp0TRAGsoft.pfx" /p "%WIN_CERT_PASSWORD%" /t http://tsa.starfieldtech.com %programNameArgument% "%~1"
if errorlevel 0 goto:eof
echo "Signin failed. Using fallback timestamp server 4!"
signtool sign /fd sha1 /f "%~dp0TRAGsoft.pfx" /p "%WIN_CERT_PASSWORD%" /t http://timestamp.geotrust.com/tsa %programNameArgument% "%~1"
if errorlevel 0 goto:eof
...
echo.
echo Usage:    TRAGsoft.Sign.bat [FilePath] [ProgramName]
