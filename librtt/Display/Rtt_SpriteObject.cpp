...
SpriteObjectSequence::Create(Rtt_Allocator *allocator, lua_State *L, int index) {
	...
	if (start > 0) {
		result = Rtt_NEW(allocator, SpriteObjectSequence(allocator, name, time, timeArray, start - 1, numFrames, loopCount));
	} else if (frames != NULL) {
		result = Rtt_NEW(allocator, SpriteObjectSequence(allocator, name, time, timeArray, frames, numFrames, loopCount));
	} else {
	...
	if (result) {
		ImageSheetUserdata *sheetUserData = NULL;
		
		lua_getfield(L, index, "sheet");
		if (lua_isuserdata(L, -1)) {
			sheetUserData = ImageSheet::ToUserdata(L, -1);
		}
		lua_pop(L, 1);
		...
		if (sheetUserData) {
			...
			const AutoPtr<ImageSheet> &sheet = sheetUserData->GetSheet();
...
void SpriteObjectSequence::Initialize(Rtt_Allocator *pAllocator, const AutoPtr<ImageSheet> &sheet) {
...
void SpriteObjectSequence::Verify(const SpriteObject &owner) const {
...
int SpriteObjectSequence::GetTimeForFrame(int frameIndex) const {
...
SpriteObjectSequence::GetEffectiveFrame(int index, SpriteEvent::Phase *phase ) const {
	...
	if (index >= fNumFrames) {
		index = index % fNumFrames; // Map index into [0, fNumFrames)
	}
...
int SpriteObjectSequence::GetEffectiveNumFrames() const {
...
void SpriteObject::Initialize(Rtt_Allocator *pAllocator) {
	Rtt_ASSERT(GetStage());
...
void SpriteObject::SetBitmapFrame(int frameIndex) {
...
void SpriteObject::ResetTimeArrayIteratorCache(SpriteObjectSequence *sequence) {
...
int SpriteObject::GetFrameIndexForDeltaTime(Real dt, SpriteObjectSequence *sequence, int effectiveNumFrames) {
	if (sequence->GetTimeArray() == NULL) {
		...
		for (int i = fTimeArrayCachedFrame; (0 == sequence->GetLoopCount() && dt > fTimeArrayCachedNextFrameTime) || i < effectiveNumFrames; ++i) {
			...
			int nextFrame = fTimeArrayCachedFrame % numFrames;
...
void SpriteObject::Update(lua_State *L, U64 milliseconds) {
	...
	if (nextFrame >= 0 && nextFrame < effectiveNumFrames) {
		...
		if (shouldDispatch && SpriteEvent::kNext == nextPhase && defaultPhase != SpriteEvent::kNext) { // TODO: && didLoop
			nextPhase = SpriteEvent::kLoop;
		}
	}
...
void SpriteObject::SetFrame(int index) {
	SpriteObjectSequence *sequence = GetCurrentSequence();
...
int SpriteObject::GetFrame() const {
	int result = 0;
...
int SpriteObject::GetNumFrames() const {
	return GetCurrentSequence()->GetNumFrames();
}
...
bool SpriteObject::IsPlaying() const {
	...
	if (result) { // For finite loops, fCurrentFrame will eventually be out of range
		result = (fCurrentFrame < GetCurrentSequence()->GetEffectiveNumFrames());
	}
	return result;
}

void SpriteObject::SetPlaying(bool newValue) {
